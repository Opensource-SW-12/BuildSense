...
| `src/pricing/passmark_tiering.py` | `cpu_passmark_static.json`/`gpu_passmark_static.json`(KAN-166, CPU 294개/GPU 368개) 정적 PassMark DB 로드(`load_cpu_passmark_items()`/`load_gpu_passmark_items()`) + tier 계산(`calculate_cpu_tier`/`calculate_gpu_tier`, 공식 `int((score/max)*29)`, CPU max=80000/GPU max=41588) |
...
**티어 데이터:** `data/chipset_tiers.json`이나 specs DB의 `performance_tier` 필드는 없음 — `chipset_tier_mapper.map_hardware_to_tiers()`가 현재 하드웨어 이름을 정적 PassMark DB(`cpu_passmark_static.json`/`gpu_passmark_static.json`, KAN-166)와 토큰 일치율(`_MIN_TOKEN_OVERLAP=0.7`)로 매칭해 tier를 즉석 계산. 수동 관리 불필요.
...
- KAN-109 (PR #81): passmark_tiering.py — PassMark 크롤링 기반 tier 산출 (KAN-166에서 정적 DB로 대체됨)
...
- KAN-166: PassMark 정적 DB(`cpu_passmark_static.json`/`gpu_passmark_static.json`) 구축, 실시간 크롤링(403 차단) 제거 — `passmark_tiering.py`를 정적 DB 로드 전용으로 단순화
