...
### 작업 중 — KAN-150-motherboard-in-settings-parts (현재 브랜치)
- 메인보드를 초기 설정 화면의 "추천 부품 선택"에 추가, 부품 옵션을 추천/유지 2가지로 통일 (제외·이미 결정·수동 입력 제거)
- **현재 진행 중: UI 다크 테마 전면 재설계** (`src/app.py`, `src/recommendation/user_input_dialog.py`, `src/settings.py` — 미커밋, UI 작업 완료 후 일괄 커밋 예정)
